...
CheMPS2::TensorL::TensorL(const int indexIn, const int IdiffIn, const bool movingRightIn, const SyBookkeeper * denBKIn) :
...
void CheMPS2::TensorL::makenewRight(TensorT * denT){
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      int IDR = Irreps::directProd( n_irrep, sectorI1[ikappa] );
      ...
      for (int geval=0; geval<2; geval++){
         ...
         if (dimL>0){
            ...
            double * BlockTdown = denT->gStorage(NL,TwoSL,IL,sectorN1[ikappa]+1,sector_2S_down[ikappa],IDR);
            ...
            if (geval>=1){
               ...
               alpha = fase * sqrt((sectorTwoS1[ikappa]+1.0)/(sector_2S_down[ikappa]+1.0));
            }
            double beta = 1.0; //add
...
void CheMPS2::TensorL::makenewLeft(TensorT * denT){
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      int IDL = Irreps::directProd( n_irrep, sectorI1[ikappa] );
      int dimUL = denBK->gCurrentDim(index,   sectorN1[ikappa],   sectorTwoS1[ikappa],    sectorI1[ikappa]);
      int dimDL = denBK->gCurrentDim(index,   sectorN1[ikappa]+1, sector_2S_down[ikappa], IDL             );
      ...
      for (int geval=0; geval<2; geval++){
         ...
         if (dimR>0){
            ...
            double * BlockTdown = denT->gStorage(sectorN1[ikappa]+1, sector_2S_down[ikappa], IDL,              NR, TwoSR, IR);
