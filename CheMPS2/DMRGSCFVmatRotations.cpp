            ...
            if ( irrep4 >= irrep2 ){
               ...
               if (( NEW1 > 0 ) && ( NEW2 > 0 ) && ( NEW3 > 0 ) && ( NEW4 > 0 )){
                  ...
                  if (( block_size1 >= second_old ) && ( block_size2 >= first_new )){ // Do everything in memory
                     ...
                     for ( int cnt4 = 0; cnt4 < ORIG4; cnt4++ ){
                        for ( int cnt3 = 0; cnt3 < ORIG3; cnt3++ ){
                           for ( int cnt2 = 0; cnt2 < ORIG2; cnt2++ ){
                              for ( int cnt1 = 0; cnt1 < ORIG1; cnt1++ ){
                                 mem1[ cnt1 + ORIG1 * ( cnt2 + ORIG2 * ( cnt3 + ORIG3 * cnt4 ) ) ]
                     ...
                     blockwise_first(  mem1, mem2, ORIG1, ORIG2, ORIG3, ORIG4, umat1, NEW1, ORIG1 );
                     blockwise_fourth( mem2, mem1,  NEW1, ORIG2, ORIG3, ORIG4, umat4, NEW4, ORIG4 );
                     blockwise_third(  mem1, mem2,  NEW1, ORIG2, ORIG3,  NEW4, umat3, NEW3, ORIG3 );
                     blockwise_second( mem2, mem1,  NEW1, ORIG2,  NEW3,  NEW4, umat2, NEW2, ORIG2 );
                           ...
                           for ( int cnt2 = 0; cnt2 < NEW2; cnt2++ ){
                              for ( int cnt1 = 0; cnt1 < NEW1; cnt1++ ){
                                 NEW_VMAT->set( irrep1, irrep2, irrep3, irrep4, cnt1, cnt2, cnt3, cnt4,
                                    mem1[ cnt1 + NEW1 * ( cnt2 + NEW2 * ( cnt3 + NEW3 * cnt4 ) ) ] );
                              }
                           }
                        }
                     }
            ...
            if ( Ia1 <= Ia2 ){
               ...
               if (( NEW_C1 > 0 ) && ( NEW_C2 > 0 ) && ( NEW_A1 > 0 ) && ( NEW_A2 > 0 )){
                  ...
                  if (( block_size1 >= second_old ) && ( block_size2 >= first_new )){ // Do everything in memory
                        ...
                        for ( int a2 = 0; a2 < NORB_A2; a2++ ){
                           for ( int a1 = 0; a1 < NORB_A1; a1++ ){
                              for ( int c1 = 0; c1 < NORB_C1; c1++ ){
                                 ...
                                 mem1[ c1 + NORB_C1 * ( a1 + NORB_A1 * ( a2 + NORB_A2 * c2 ) ) ]
                     ...
                     blockwise_first(  mem1, mem2, NORB_C1, NORB_A1, NORB_A2, NORB_C2, umat_C1, NEW_C1, NORB_C1 );
                     blockwise_fourth( mem2, mem1,  NEW_C1, NORB_A1, NORB_A2, NORB_C2, umat_C2, NEW_C2, NORB_C2 );
                     blockwise_third(  mem1, mem2,  NEW_C1, NORB_A1, NORB_A2,  NEW_C2, umat_A2, NEW_A2, NORB_A2 );
                     blockwise_second( mem2, mem1,  NEW_C1, NORB_A1,  NEW_A2,  NEW_C2, umat_A1, NEW_A1, NORB_A1 );
                           ...
                           for ( int a1 = 0; a1 < NEW_A1; a1++ ){
                              for ( int c1 = 0; c1 < NEW_C1; c1++ ){
                                 ROT_TEI->set_coulomb( Ic1, Ic2, Ia1, Ia2, c1, c2, a1, a2,
                                    mem1[ c1 + NEW_C1 * ( a1 + NEW_A1 * ( a2 + NEW_A2 * c2 ) ) ] );
                              }
                           }
                        }
                     }
