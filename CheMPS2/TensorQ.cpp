...
CheMPS2::TensorQ::TensorQ(const int indexIn, const int IdiffIn, const bool movingRightIn, const SyBookkeeper * denBKIn, const Problem * ProbIn, const int siteIn) :
...
void CheMPS2::TensorQ::AddTermSimple(TensorT * denT){
   ...
   if (( moving_right) && (denBK->gIrrep(denT->gIndex()) == n_irrep)){ AddTermSimpleRight(denT); }
   if ((!moving_right) && (denBK->gIrrep(denT->gIndex()) == n_irrep)){ AddTermSimpleLeft( denT); }
...
void CheMPS2::TensorQ::AddTermSimpleRight(TensorT * denT){
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      const int ID = Irreps::directProd( n_irrep, sectorI1[ikappa] );
      int dimRU = denBK->gCurrentDim(index,   sectorN1[ikappa],   sectorTwoS1[ikappa],    sectorI1[ikappa]);
      int dimRD = denBK->gCurrentDim(index,   sectorN1[ikappa]+1, sector_2S_down[ikappa], ID);
      int dimL  = denBK->gCurrentDim(index-1, sectorN1[ikappa]-1, sector_2S_down[ikappa], ID);
      if (dimL>0){
      
         double * BlockTup = denT->gStorage(sectorN1[ikappa]-1, sector_2S_down[ikappa], ID, sectorN1[ikappa],   sectorTwoS1[ikappa],    sectorI1[ikappa]);
         double * BlockTdo = denT->gStorage(sectorN1[ikappa]-1, sector_2S_down[ikappa], ID, sectorN1[ikappa]+1, sector_2S_down[ikappa], ID);
         
         int fase = ((((sector_2S_down[ikappa]+1-sectorTwoS1[ikappa])/2)%2)!=0)?-1:1;
         double alpha = fase * mxElement * sqrt((sectorTwoS1[ikappa]+1.0)/(sector_2S_down[ikappa]+1.0));
         double beta = 1.0; //add
         char trans = 'T';
         char notr = 'N';
...
void CheMPS2::TensorQ::AddTermSimpleLeft(TensorT * denT){
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      ...
      int dimLD = denBK->gCurrentDim(index,   sectorN1[ikappa]+1, sector_2S_down[ikappa], ID);
      ...
      if (dimR>0){
         ...
         double * BlockTdo = denT->gStorage(sectorN1[ikappa]+1, sector_2S_down[ikappa], ID,               sectorN1[ikappa]+2, sectorTwoS1[ikappa], sectorI1[ikappa]);
         
         int fase = ((((sectorTwoS1[ikappa]+1-sector_2S_down[ikappa])/2)%2)!=0)?-1:1;
         double alpha = fase * mxElement * sqrt((sectorTwoS1[ikappa]+1.0)/(sector_2S_down[ikappa]+1.0));
         double beta = 1.0; //add
         char trans = 'T';
         char notr = 'N';
...
void CheMPS2::TensorQ::AddTermsLRight(TensorL ** Ltensors, TensorT * denT, double * workmem, double * workmem2){
   ...
   if (OneToAdd){
      for (int ikappa=0; ikappa<nKappa; ikappa++){
         ...
         int dimLD = denBK->gCurrentDim(index-1, sectorN1[ikappa]-1, sector_2S_down[ikappa], ID);
         ...
         if ((dimLU>0) && (dimLD>0)){
            ...
            for (int loca=0; loca<index-1; loca++){
               if (Ltensors[index-2-loca]->get_irrep() == n_irrep){
                  double * BlockL = Ltensors[index-2-loca]->gStorage(sectorN1[ikappa]-1,sector_2S_down[ikappa],ID,sectorN1[ikappa],sectorTwoS1[ikappa],sectorI1[ikappa]);
            ...
            int fase = ((((sector_2S_down[ikappa]+1-sectorTwoS1[ikappa])/2)%2)!=0)?-1:1;
            double alpha = fase * sqrt((sectorTwoS1[ikappa]+1.0)/(sector_2S_down[ikappa]+1.0));
            double beta = 0.0; //set
         ...
         if ((dimLU>0) && (dimLD>0)){
            ...
            for (int loca=0; loca<index-1; loca++){
               if (Ltensors[index-2-loca]->get_irrep() == n_irrep){
                  double * BlockL = Ltensors[index-2-loca]->gStorage(sectorN1[ikappa]-2,sectorTwoS1[ikappa],sectorI1[ikappa],sectorN1[ikappa]-1,sector_2S_down[ikappa],ID);
         ...
         for (int TwoSLU=sectorTwoS1[ikappa]-1; TwoSLU<=sectorTwoS1[ikappa]+1; TwoSLU+=2){
            for (int TwoSLD=sector_2S_down[ikappa]-1; TwoSLD<=sector_2S_down[ikappa]+1; TwoSLD+=2){
               if ((TwoSLD>=0) && (TwoSLU>=0) && (abs(TwoSLD-TwoSLU)<2)){
                  ...
                  if ((dimLU>0) && (dimLD>0)){
                     ...
                     double factor = fase * sqrt((TwoSLD+1)*(sectorTwoS1[ikappa]+1.0)) * gsl_sf_coupling_6j(sectorTwoS1[ikappa], sector_2S_down[ikappa], 1, TwoSLD, TwoSLU, 1);
                     ...
                     double * BlockTdo = denT->gStorage(sectorN1[ikappa]  , TwoSLD, ILD, sectorN1[ikappa]+1, sector_2S_down[ikappa], ID);
...
void CheMPS2::TensorQ::AddTermsLLeft(TensorL ** Ltensors, TensorT * denT, double * workmem, double * workmem2){
   ...
   if (OneToAdd){
      for (int ikappa=0; ikappa<nKappa; ikappa++){
         ...
         int dimRD = denBK->gCurrentDim(index+1, sectorN1[ikappa]+1, sector_2S_down[ikappa], ID);
         ...
         if ((dimRU>0) && (dimRD>0)){
            ...
            for (int loca=index+1; loca<Prob->gL(); loca++){
               if (Ltensors[loca-index-1]->get_irrep() == n_irrep){
                  double * BlockL = Ltensors[loca-index-1]->gStorage(sectorN1[ikappa]+1,sector_2S_down[ikappa],ID,sectorN1[ikappa]+2,sectorTwoS1[ikappa],sectorI1[ikappa]);
         ...
         //case 2
         dimRD = denBK->gCurrentDim(index+1, sectorN1[ikappa]+3, sector_2S_down[ikappa], ID);
         //dimRU same as case1
         if ((dimRU>0) && (dimRD>0)){
            ...
            for (int loca=index+1; loca<Prob->gL(); loca++){
               if (Ltensors[loca-index-1]->get_irrep() == n_irrep){
                  double * BlockL = Ltensors[loca-index-1]->gStorage(sectorN1[ikappa]+2,sectorTwoS1[ikappa],sectorI1[ikappa],sectorN1[ikappa]+3,sector_2S_down[ikappa],ID);
            ...
            double * BlockTup = denT->gStorage(sectorN1[ikappa],   sectorTwoS1[ikappa],    sectorI1[ikappa], sectorN1[ikappa]+2, sectorTwoS1[ikappa],    sectorI1[ikappa]);
            double * BlockTdo = denT->gStorage(sectorN1[ikappa]+1, sector_2S_down[ikappa], ID,               sectorN1[ikappa]+3, sector_2S_down[ikappa], ID);
         ...
         for (int TwoSRU=sectorTwoS1[ikappa]-1; TwoSRU<=sectorTwoS1[ikappa]+1; TwoSRU+=2){
            for (int TwoSRD=sector_2S_down[ikappa]-1; TwoSRD<=sector_2S_down[ikappa]+1; TwoSRD+=2){
               if ((TwoSRD>=0) && (TwoSRU>=0) && (abs(TwoSRD-TwoSRU)<2)){
                  ...
                  if ((dimRU>0) && (dimRD>0)){
                     int fase = ((((sector_2S_down[ikappa]+TwoSRU)/2)%2)!=0)?-1:1;
                     double factor1 = fase * sqrt((TwoSRU+1.0)/(sector_2S_down[ikappa]+1.0)) * (TwoSRD+1) * gsl_sf_coupling_6j(sectorTwoS1[ikappa], sector_2S_down[ikappa], 1, TwoSRD, TwoSRU, 1);
                     double factor2 = (TwoSRD+1.0)/(sector_2S_down[ikappa]+1.0);
                     ...
                     for (int loca=index+1; loca<Prob->gL(); loca++){
                        if (Ltensors[loca-index-1]->get_irrep() == n_irrep){
                           ...
                           if (TwoSRU==sector_2S_down[ikappa]){ alpha += factor2 * Prob->gMxElement(site,index,index,loca); }
                           int inc = 1;
                           daxpy_(&dimRUxRD, &alpha, BlockL, &inc, workmem, &inc);
                        }
                     }
                     ...
                     double * BlockTdo = denT->gStorage(sectorN1[ikappa]+1, sector_2S_down[ikappa], ID,               sectorN1[ikappa]+2, TwoSRD, IRD);
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      ...
      for (int TwoSLU=sectorTwoS1[ikappa]-1; TwoSLU<=sectorTwoS1[ikappa]+1; TwoSLU+=2){
         ...
         if ((dimLU>0) && (dimLD>0)){
            ...
            int fase = ((((TwoSLU + sector_2S_down[ikappa] + 2)/2)%2)!=0)?-1:1;
            double factorB = fase * sqrt(3.0*(sectorTwoS1[ikappa]+1)) * gsl_sf_coupling_6j(1,2,1,sector_2S_down[ikappa],sectorTwoS1[ikappa],TwoSLU);
            
            double alpha;
            double * mem;
            
            if (TwoSLU == sector_2S_down[ikappa]){
            
               fase = ((((sector_2S_down[ikappa]+1-sectorTwoS1[ikappa])/2)%2)!=0)?-1:1;
               double factorA = fase * sqrt( 0.5 * (sectorTwoS1[ikappa]+1.0) / (sector_2S_down[ikappa]+1.0) );
            
               double * BlockA = denA->gStorage( sectorN1[ikappa]-1,TwoSLU,ILU,sectorN1[ikappa]+1,sector_2S_down[ikappa],ID );
               double * BlockB = denB->gStorage( sectorN1[ikappa]-1,TwoSLU,ILU,sectorN1[ikappa]+1,sector_2S_down[ikappa],ID );
               ...
               alpha = factorB;
               mem = denB->gStorage(sectorN1[ikappa]-1,TwoSLU,ILU,sectorN1[ikappa]+1,sector_2S_down[ikappa],ID);
      ...
      for (int TwoSLD=sector_2S_down[ikappa]-1; TwoSLD<=sector_2S_down[ikappa]+1; TwoSLD+=2){
         ...
         if ((dimLU>0) && (dimLD>0)){
            ...
            int fase = ((((sectorTwoS1[ikappa] + sector_2S_down[ikappa] + 1)/2)%2)!=0)?-1:1;
            double factorB = fase * sqrt(3.0*(TwoSLD+1)) * gsl_sf_coupling_6j(1,2,1,sectorTwoS1[ikappa],sector_2S_down[ikappa],TwoSLD);
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      ...
      for (int TwoSRD=sector_2S_down[ikappa]-1; TwoSRD<=sector_2S_down[ikappa]+1; TwoSRD+=2){
         ...
         if ((dimRU>0) && (dimRD>0)){
            ...
            double factorB = fase * sqrt(3.0/(sector_2S_down[ikappa]+1.0)) * (TwoSRD+1) * gsl_sf_coupling_6j(1,1,2,sectorTwoS1[ikappa],TwoSRD,sector_2S_down[ikappa]);
            ...
            if (TwoSRD == sectorTwoS1[ikappa]){
               ...
               fase = ((((sectorTwoS1[ikappa]+1-sector_2S_down[ikappa])/2)%2)!=0)?-1:1;
               double factorA = fase * sqrt( 0.5 * (sectorTwoS1[ikappa]+1.0) / (sector_2S_down[ikappa]+1.0) );
      ...
      for (int TwoSRU=sectorTwoS1[ikappa]-1; TwoSRU<=sectorTwoS1[ikappa]+1; TwoSRU+=2){
         ...
         int dimRD = denBK->gCurrentDim(index+1, sectorN1[ikappa]+3, sector_2S_down[ikappa], ID);
         if ((dimRU>0) && (dimRD>0)){
         
            int fase = ((((sectorTwoS1[ikappa] + sector_2S_down[ikappa] + 1)/2)%2)!=0)?-1:1;
            double factorB = fase * sqrt(3.0*(TwoSRU+1)) * gsl_sf_coupling_6j(1,1,2,TwoSRU,sector_2S_down[ikappa],sectorTwoS1[ikappa]);
            
            double alpha;
            double * mem;
            
            if (TwoSRU == sector_2S_down[ikappa]){

               double factorA = - sqrt(0.5);
            
               double * BlockA = denA->gStorage( sectorN1[ikappa]+1, TwoSRU, IRU, sectorN1[ikappa]+3, sector_2S_down[ikappa], ID );
               double * BlockB = denB->gStorage( sectorN1[ikappa]+1, TwoSRU, IRU, sectorN1[ikappa]+3, sector_2S_down[ikappa], ID );
               ...
               alpha = factorB;
               mem = denB->gStorage( sectorN1[ikappa]+1, TwoSRU, IRU, sectorN1[ikappa]+3, sector_2S_down[ikappa], ID );
...
void CheMPS2::TensorQ::AddTermsCDRight(TensorOperator * denC, TensorOperator * denD, TensorT * denT, double * workmem, double * workmem2){
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      const int IRD  = Irreps::directProd( sectorI1[ikappa], n_irrep );
      int dimRU = denBK->gCurrentDim(index, sectorN1[ikappa],   sectorTwoS1[ikappa],    sectorI1[ikappa]);
      int dimRD = denBK->gCurrentDim(index, sectorN1[ikappa]+1, sector_2S_down[ikappa], IRD);
      ...
      for (int TwoSLD=sector_2S_down[ikappa]-1; TwoSLD<=sector_2S_down[ikappa]+1; TwoSLD+=2){
         ...
         if ((dimLU>0) && (dimLD>0)){
            ...
            //first set to D
            int fase = ((((sectorTwoS1[ikappa]+sector_2S_down[ikappa]+1)/2)%2)!=0)?-1:1;
            double factor = fase * sqrt(3.0*(TwoSLD+1)) * gsl_sf_coupling_6j(1,2,1,sectorTwoS1[ikappa],sector_2S_down[ikappa],TwoSLD);
            ...
            double * BlockTup = denT->gStorage( sectorN1[ikappa], sectorTwoS1[ikappa], sectorI1[ikappa], sectorN1[ikappa], sectorTwoS1[ikappa], sectorI1[ikappa] );
            double * BlockTdo = denT->gStorage( sectorN1[ikappa], TwoSLD, ILD, sectorN1[ikappa]+1, sector_2S_down[ikappa], IRD );
      ...
      for (int TwoSLU=sectorTwoS1[ikappa]-1; TwoSLU<=sectorTwoS1[ikappa]+1; TwoSLU+=2){
         int dimLU = denBK->gCurrentDim(index-1, sectorN1[ikappa]-1, TwoSLU,                 ILU);
         int dimLD = denBK->gCurrentDim(index-1, sectorN1[ikappa]-1, sector_2S_down[ikappa], IRD);
         ...
         if ((dimLU>0) && (dimLD>0)){
            ...
            //first set to D
            int fase = ((((TwoSLU + sector_2S_down[ikappa])/2)%2)!=0)?-1:1;
            double factor = fase * sqrt(3.0*(sectorTwoS1[ikappa]+1)) * gsl_sf_coupling_6j(1,2,1,sector_2S_down[ikappa],sectorTwoS1[ikappa],TwoSLU);
            double * block = denD->gStorage( sectorN1[ikappa]-1, TwoSLU, ILU, sectorN1[ikappa]-1, sector_2S_down[ikappa], IRD );
            for (int cnt=0; cnt<dimLUxLD; cnt++){ workmem[cnt] = factor * block[cnt]; }
            
            //add C
            if (TwoSLU==sector_2S_down[ikappa]){
               fase = ((((sector_2S_down[ikappa]+1-sectorTwoS1[ikappa])/2)%2)!=0)?-1:1;
               factor = fase * sqrt(0.5 * ( sectorTwoS1[ikappa]+1.0 ) / ( sector_2S_down[ikappa] + 1.0 ) );
               block = denC->gStorage( sectorN1[ikappa]-1, TwoSLU, ILU, sectorN1[ikappa]-1, sector_2S_down[ikappa], IRD );
               int inc = 1;
               daxpy_(&dimLUxLD, &factor, block, &inc, workmem, &inc);
            }
            
            double * BlockTup = denT->gStorage( sectorN1[ikappa]-1, TwoSLU,                 ILU, sectorN1[ikappa],   sectorTwoS1[ikappa],    sectorI1[ikappa] );
            double * BlockTdo = denT->gStorage( sectorN1[ikappa]-1, sector_2S_down[ikappa], IRD, sectorN1[ikappa]+1, sector_2S_down[ikappa], IRD );
...
void CheMPS2::TensorQ::AddTermsCDLeft(TensorOperator * denC, TensorOperator * denD, TensorT * denT, double * workmem, double * workmem2){
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      const int ILD  = Irreps::directProd( sectorI1[ikappa], n_irrep );
      int dimLU = denBK->gCurrentDim(index, sectorN1[ikappa],   sectorTwoS1[ikappa],    sectorI1[ikappa]);
      int dimLD = denBK->gCurrentDim(index, sectorN1[ikappa]+1, sector_2S_down[ikappa], ILD);
   
      //case 1
      const int IRU = Irreps::directProd(sectorI1[ikappa],denBK->gIrrep(index));
      for (int TwoSRU=sectorTwoS1[ikappa]-1; TwoSRU<=sectorTwoS1[ikappa]+1; TwoSRU+=2){
         int dimRU = denBK->gCurrentDim(index+1, sectorN1[ikappa]+1, TwoSRU,                 IRU);
         int dimRD = denBK->gCurrentDim(index+1, sectorN1[ikappa]+1, sector_2S_down[ikappa], ILD);
         ...
         if ((dimRU>0) && (dimRD>0)){
            ...
            double factor = fase * sqrt(3.0/(sector_2S_down[ikappa]+1.0)) * (TwoSRU+1) * gsl_sf_coupling_6j(1,1,2,TwoSRU,sector_2S_down[ikappa],sectorTwoS1[ikappa]);
            double * block = denD->gStorage( sectorN1[ikappa]+1, TwoSRU, IRU, sectorN1[ikappa]+1, sector_2S_down[ikappa], ILD );
            for (int cnt=0; cnt<dimRUxRD; cnt++){ workmem[cnt] = factor * block[cnt]; }
            
            //add C
            if (TwoSRU==sector_2S_down[ikappa]){
               factor = sqrt(0.5);
               block = denC->gStorage( sectorN1[ikappa]+1, TwoSRU, IRU, sectorN1[ikappa]+1, sector_2S_down[ikappa], ILD );
               int inc = 1;
               daxpy_(&dimRUxRD, &factor, block, &inc, workmem, &inc);
            }
            
            double * BlockTup = denT->gStorage( sectorN1[ikappa],   sectorTwoS1[ikappa],    sectorI1[ikappa], sectorN1[ikappa]+1, TwoSRU,                 IRU );
            double * BlockTdo = denT->gStorage( sectorN1[ikappa]+1, sector_2S_down[ikappa], ILD,              sectorN1[ikappa]+1, sector_2S_down[ikappa], ILD );
      ...
      for (int TwoSRD=sector_2S_down[ikappa]-1; TwoSRD<=sector_2S_down[ikappa]+1; TwoSRD+=2){
         ...
         if ((dimRU>0) && (dimRD>0)){
            ...
            double factor = fase * sqrt(3.0*(TwoSRD+1.0)*(sectorTwoS1[ikappa]+1.0)/(sector_2S_down[ikappa]+1.0)) * gsl_sf_coupling_6j(1,1,2,sectorTwoS1[ikappa],TwoSRD,sector_2S_down[ikappa]);
            ...
            double * BlockTup = denT->gStorage( sectorN1[ikappa],  sectorTwoS1[ikappa],   sectorI1[ikappa],sectorN1[ikappa]+2,sectorTwoS1[ikappa],sectorI1[ikappa] );
            double * BlockTdo = denT->gStorage( sectorN1[ikappa]+1,sector_2S_down[ikappa],ILD,             sectorN1[ikappa]+2,TwoSRD,             IRD);
