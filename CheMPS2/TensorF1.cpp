...
CheMPS2::TensorF1::TensorF1(const int boundary_index, const int Idiff, const bool moving_right, const SyBookkeeper * denBK) :
...
void CheMPS2::TensorF1::makenewRight(TensorT * denT){ //Idiff = Itrivial
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      ...
      int dimRD = bk_up->gCurrentDim(index, sector_nelec_up[ikappa], sector_spin_down[ikappa], sector_irrep_up[ikappa]);
      
      const int IL = Irreps::directProd( sector_irrep_up[ikappa] , bk_up->gIrrep(index-1) );
      for (int geval=0; geval<2; geval++){
         int TwoSL = sector_spin_up[ikappa] + ((geval==0)?-1:1);
         if ((TwoSL>=0) && (abs(sector_spin_down[ikappa]-TwoSL)<2)){
            int dimL  = bk_up->gCurrentDim(index-1, sector_nelec_up[ikappa]-1, TwoSL, IL);
            if (dimL>0){
         
               double * BlockTup = denT->gStorage(sector_nelec_up[ikappa]-1, TwoSL, IL, sector_nelec_up[ikappa], sector_spin_up[ikappa],    sector_irrep_up[ikappa]);
               double * BlockTdo = denT->gStorage(sector_nelec_up[ikappa]-1, TwoSL, IL, sector_nelec_up[ikappa], sector_spin_down[ikappa], sector_irrep_up[ikappa]);

               char trans = 'T';
               char notrans = 'N';
               int fase = ((((TwoSL + sector_spin_down[ikappa] + 3)/2)%2)!=0)?-1:1;
               double alpha = fase * sqrt(3.0*(sector_spin_up[ikappa]+1)) * gsl_sf_coupling_6j(1,1,2,sector_spin_up[ikappa],sector_spin_down[ikappa],TwoSL);
               double beta = 1.0; //add
...
void CheMPS2::TensorF1::makenewLeft(TensorT * denT){ //Idiff = Itrivial
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      ...
      int dimLD = bk_up->gCurrentDim(index, sector_nelec_up[ikappa], sector_spin_down[ikappa], sector_irrep_up[ikappa]);

      const int IR = Irreps::directProd( sector_irrep_up[ikappa] , bk_up->gIrrep(index) );
      for (int geval=0; geval<2; geval++){
         int TwoSR = sector_spin_up[ikappa] + ((geval==0)?-1:1);
         if ((TwoSR>=0) && (abs(sector_spin_down[ikappa]-TwoSR)<2)){
            int dimR  = bk_up->gCurrentDim(index+1, sector_nelec_up[ikappa]+1, TwoSR, IR);
            if (dimR>0){
         
               double * BlockTup   = denT->gStorage(sector_nelec_up[ikappa], sector_spin_up[ikappa],    sector_irrep_up[ikappa], sector_nelec_up[ikappa]+1, TwoSR, IR);
               double * BlockTdown = denT->gStorage(sector_nelec_up[ikappa], sector_spin_down[ikappa], sector_irrep_up[ikappa], sector_nelec_up[ikappa]+1, TwoSR, IR);

               char trans = 'T';
               char notrans = 'N';
               int fase = ((((sector_spin_down[ikappa] + TwoSR + 1)/2)%2)!=0)?-1:1;
               double alpha = fase * sqrt(3.0/(sector_spin_up[ikappa]+1.0)) * (TwoSR + 1) * gsl_sf_coupling_6j(1,1,2,sector_spin_up[ikappa],sector_spin_down[ikappa],TwoSR);
               double beta = 1.0; //add
...
void CheMPS2::TensorF1::makenewRight(TensorL * denL, TensorT * denT, double * workmem){
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      ...
      int dimDR = bk_up->gCurrentDim(index, sector_nelec_up[ikappa], sector_spin_down[ikappa], IDR             );
      ...
      for (int geval=0; geval<4; geval++){
         ...
         if ((dimLU>0) && (dimLD>0) && (abs(TwoSLU-TwoSLD)<2)){
            ...
            double * BlockTdown = denT->gStorage(NLU+1, TwoSLD, ILD, sector_nelec_up[ikappa], sector_spin_down[ikappa], IDR             );
            ...
            double alpha;
            if (geval<=1){
               int fase = ((((TwoSLU + sector_spin_down[ikappa] + 3)/2)%2)!=0)?-1:1;
...
void CheMPS2::TensorF1::makenewLeft(TensorL * denL, TensorT * denT, double * workmem){
   ...
   for (int ikappa=0; ikappa<nKappa; ikappa++){
      ...
      int dimDL = bk_up->gCurrentDim(index, sector_nelec_up[ikappa], sector_spin_down[ikappa], IDL             );
      ...
      for (int geval=0; geval<4; geval++){
         ...
         if ((dimRU>0) && (dimRD>0) && (abs(TwoSRU-TwoSRD)<2)){
            ...
            double * BlockTdown = denT->gStorage(sector_nelec_up[ikappa], sector_spin_down[ikappa], IDL,              NRU+1, TwoSRD, IRD);
            ...
            double alpha;
            if (geval<=1){
               int fase = ((((sector_spin_down[ikappa] + TwoSRD + 1)/2)%2)!=0)?-1:1;
               alpha = fase * sqrt(3.0/(sector_spin_up[ikappa]+1.0)) * (TwoSRD+1) * gsl_sf_coupling_6j(1,1,2,sector_spin_up[ikappa],sector_spin_down[ikappa],TwoSRD);
            } else {
               int fase = (((sector_spin_up[ikappa])%2)!=0)?-1:1;
               alpha = fase * sqrt(3.0 *(TwoSRU+1.0)*(sector_spin_down[ikappa]+1.0)/(sector_spin_up[ikappa]+1.0)) * gsl_sf_coupling_6j(1,1,2,sector_spin_up[ikappa],sector_spin_down[ikappa],TwoSRU);
            }
            double beta = 0.0; //set
